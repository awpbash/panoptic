...
def open_image(path_or_obj):
    # 1) Already a NumPy image (OpenCV BGR)
    if isinstance(path_or_obj, np.ndarray):
        import cv2
        rgb = cv2.cvtColor(path_or_obj, cv2.COLOR_BGR2RGB)
        return Image.fromarray(rgb)

    # 2) Bytes/bytearray buffer
    if isinstance(path_or_obj, (bytes, bytearray)):
        return Image.open(io.BytesIO(path_or_obj)).convert('RGB')

    # # 3) String path or URL
    # if isinstance(path_or_obj, str):
    #     if path_or_obj.startswith('http://') or path_or_obj.startswith('https://'):
    #         resp = requests.get(path_or_obj, stream=True)
    #         resp.raise_for_status()
    #         return Image.open(io.BytesIO(resp.content)).convert('RGB')
    #     if os.path.exists(path_or_obj):
    #         return Image.open(path_or_obj).convert('RGB')
    #     raise FileNotFoundError(f"The file {path_or_obj} does not exist.")

    raise TypeError(f"Unsupported input type for open_image: {type(path_or_obj)}")
...
class RTDETR_ONNX:
    ...
    def __init__(self, onnx_model_path):
        ...
        ih, iw = input_info.shape[2], input_info.shape[3]
        # some models are dynamic (None); default to 640
        self.input_height = ih if isinstance(ih, int) else 640
        self.input_width  = iw if isinstance(iw, int) else 640
